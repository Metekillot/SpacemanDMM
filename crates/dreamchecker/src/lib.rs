        ...
        for proc in ty.iter_self_procs() {
            if let Some(ref code) = proc.get().code {
                if let Some(refcell) = annotation_tree {
                    analyzer.check_proc(proc, code, Some(refcell));
                } else {
...
impl<'o> AnalyzeObjectTree<'o> {
    ...
    pub fn check_proc(
        &mut self,
        proc: ProcRef<'o>,
        code: &'o [Spanned<Statement>],
        annotate_to: Option<&RefCell<AnnotationTree>>,
    ) {
...
impl<'o, 's> AnalyzeProc<'o, 's> {
    ...
    pub fn run(
        ...
        self.visit_block(block, &mut local_vars, annotate_to);
    ...
    fn visit_block(
        ...
        if annotate_to.is_some() {
            for statement_pair in block.chunks(2) {
                ...
                let state = self.visit_statement(
                    stmt.location,
                    &stmt.elem,
                    local_vars,
                    next_stmt,
            ...
            for stmt in block.iter() {
                ...
                let state =
                    self.visit_statement(stmt.location, &stmt.elem, local_vars, None, annotate_to);
                term.merge(state);
            }
        }
        term
    ...
    fn visit_statement(
        ...
        local_vars: &mut HashMap<String, LocalVar<'o>>,
        next_statement: Option<&Spanned<Statement>>,
        annotate_to: Option<&RefCell<AnnotationTree>>,
    ) -> ControlFlow {
        match statement {
            Statement::Expr(expr) => {
            ...
            Statement::While { condition, block } => {
                ...
                let mut state = self.visit_block(block, &mut scoped_locals, annotate_to);
                state.end_loop();
                return state;
            }
            ...
            Statement::If { arms, else_arm } => {
                ...
                for (condition, ref block) in arms.iter() {
                    ...
                    let state = self.visit_block(block, &mut scoped_locals, annotate_to);
                ...
                if let Some(else_arm) = else_arm {
                    ...
                    let state = self.visit_block(else_arm, &mut local_vars.clone(), annotate_to);
                    allterm.merge_false(state);
                } else {
                    allterm.no_else();
                    return allterm;
            ...
            Statement::ForLoop {
                ...
                if let Some(init) = init {
                    ...
                    self.visit_statement(location, init, &mut scoped_locals, None, annotate_to);
                }
                ...
                if let Some(inc) = inc {
                    self.visit_statement(location, inc, &mut scoped_locals, None, annotate_to);
                }
                let mut state = self.visit_block(block, &mut scoped_locals, annotate_to);
                state.end_loop();
                return state;
            ...
            Statement::Spawn { delay, block } => {
                ...
                self.visit_block(block, &mut scoped_locals, annotate_to);
            ...
            Statement::Switch {
                ...
                if let Some(default) = default {
                    let state = self.visit_block(default, &mut local_vars.clone(), annotate_to);
                    allterm.merge_false(state);
                } else {
                    allterm.no_else();
                    return allterm;
            ...
            Statement::TryCatch {
                ...
                self.visit_block(try_block, &mut local_vars.clone(), annotate_to);
                ...
                self.visit_block(catch_block, &mut catch_locals, annotate_to);
            }
            ...
            Statement::Label { name: _, block } => {
                self.visit_block(block, &mut local_vars.clone(), annotate_to);
            }
            Statement::Del(expr) => {
                self.visit_expression(location, expr, None, local_vars);
            }
